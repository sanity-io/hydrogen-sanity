...
- [hydrogen-sanity](#hydrogen-sanity)
  ...
  - [Usage](#usage)
    ...
    - [Fetching Sanity data with `query`](#fetching-sanity-data-with-query)
    - [Live preview](#live-preview)
    - [Entering preview mode](#entering-preview-mode)
...
1. Update any Sanity Client fetches with `loadQuery`.
...
2. Then in the default export, pass this initial object to the `useQuery` hook imported from React Loader.
...
export async function loader({context}: LoaderArgs) {
  return json({
    // ... other loader data
    // Just check if the app is in preview mode
