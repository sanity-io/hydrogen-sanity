...
describe('stegaEnabled serialization', () => {
  ...
  it('should set stegaEnabled to true when preview is enabled', () => {
    const previewSession = new PreviewSession()
    previewSession.set('projectId', projectId)
    ...
    const providerProps = (context.SanityProvider({children: null}) as any).props
    expect(providerProps.value.stegaEnabled).toBe(true)
    expect(providerProps.value.preview).toBe(true)
  })
  ...
  it('should set stegaEnabled to false when preview is disabled', () => {
    ...
    expect(providerProps.value.preview).toBe(false)
  })
  ...
  it('should work with Hydrogen session for stegaEnabled', () => {
    ...
    const context = createSanityContext({
      ...
      preview: {
        token: 'my-token',
        session: hydrogenSession as any,
      },
    })
  ...
  it('should include stegaEnabled in provider value interface', () => {
    ...
    expect(providerValue).toHaveProperty('preview')
  ...
  it('should maintain consistency between preview and stegaEnabled flags', () => {
    const previewSession = new PreviewSession()
    previewSession.set('projectId', projectId)
    ...
    const providerPropsWithPreview = (contextWithPreview.SanityProvider({children: null}) as any)
      .props
    const providerPropsWithoutPreview = (
      contextWithoutPreview.SanityProvider({children: null}) as any
    ).props

    // Both should be true when preview is enabled
    expect(providerPropsWithPreview.value.preview).toBe(true)
    expect(providerPropsWithPreview.value.stegaEnabled).toBe(true)

    // Both should be false when preview is disabled
    expect(providerPropsWithoutPreview.value.preview).toBe(false)
